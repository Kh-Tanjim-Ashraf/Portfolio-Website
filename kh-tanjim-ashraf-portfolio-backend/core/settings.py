...
# Intialize the environment, define the type schemas & default values; [Required for Production] Hide the sensitive information to get exposed in public
env = environ.Env(
...
MIDDLEWARE = [
    ...
    # [Required for Production] WhiteNoise Middleware helps Gunicorn to serve the static files, which are generated by the `collectstatic` command, while the app is running by a production server
